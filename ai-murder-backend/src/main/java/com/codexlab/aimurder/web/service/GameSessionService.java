...
public class GameSessionService {
    ...
    public void refreshSessionState(GameSession session, String message, String speakerId) {
        ...
        applyScriptedAction(session, message);
        session.setUpdatedAt(LocalDateTime.now());
    }
    ...
    private void applyScriptedAction(GameSession session, String message) {
        if (!InMemoryScriptRepository.THIRD_SCRIPT_ID.equals(session.getScriptId())) {
            return;
        }
        String normalizedMessage = defaultText(message, "");
        ...
        int stageOrder = session.getCurrentStage() == null ? 0 : session.getCurrentStage().getStageOrder();

        if (stageOrder == 1) {
            if (containsAny(normalizedMessage, "????", "???", "??", "??")) {
                revealClueIfAvailable(session, "summer-clue-recorder-shell");
                revealClueIfAvailable(session, "summer-clue-anonymous-note");
                session.setCurrentEnvironmentSummary("????????????????????????????????????????????");
                session.setCurrentStoryBeat("??????????????????????????");
                queueSceneCue(session, SceneCueType.REACTION, "??", "???????????????????????????????", List.of("?????????????????", "?????????????????"));
            }
            if (containsAny(normalizedMessage, "??", "???", "????")) {
                revealClueIfAvailable(session, "summer-clue-corridor-argument");
                session.setCurrentEnvironmentSummary("??????????????????????????????????????????");
                queueSceneCue(session, SceneCueType.PRESSURE, "??", "???????????????????????????", List.of("????????????????????", "?????????????????"));
            }
            return;
        }
        ...
        if (stageOrder == 2) {
            if (containsAny(normalizedMessage, "???", "???", "????", "??")) {
                revealClueIfAvailable(session, "summer-clue-audio-fragment");
                revealClueIfAvailable(session, "summer-clue-duty-log");
                session.setCurrentEnvironmentSummary("?????????????????????????????????????????????");
                session.setCurrentStoryBeat("??????????????????????????????");
                queueSceneCue(session, SceneCueType.REACTION, "???", "????????????????????????????????????????", List.of("????????????????", "?????????????????"));
            }
            if (containsAny(normalizedMessage, "??", "??", "???")) {
                revealClueIfAvailable(session, "summer-clue-ripped-photo");
                queueSceneCue(session, SceneCueType.FORESHADOW, "??", "??????????????????????????????????", List.of("????????????", "???????????????"));
            }
            return;
        }
        ...
        if (stageOrder == 3) {
            if (containsAny(normalizedMessage, "??", "?????", "???")) {
                revealClueIfAvailable(session, "summer-clue-unsent-letter");
                session.setCurrentStoryBeat("??????????????????????????????????");
                queueSceneCue(session, SceneCueType.REACTION, "??", "????????????????????????????????????", List.of("?????????????????", "??????????"));
            }
            if (containsAny(normalizedMessage, "??", "???", "???")) {
                revealClueIfAvailable(session, "summer-clue-discipline-copy");
                queueSceneCue(session, SceneCueType.PRESSURE, "??", "???????????????????????????????????????", List.of("??????????????????", "???????????????"));
            }
            if (containsAny(normalizedMessage, "???", "??", "??", "????")) {
                revealClueIfAvailable(session, "summer-clue-broadcast-trash");
                queueSceneCue(session, SceneCueType.FORESHADOW, "???", "???????????????????????????", List.of("???????????", "?????????????"));
            }
            if (containsAny(normalizedMessage, "????", "????", "???")) {
                session.setCurrentEnvironmentSummary("??????????????????????????????????");
        ...
        if (containsAny(normalizedMessage, "?????", "?????", "???")) {
            revealClueIfAvailable(session, "summer-clue-teacher-note");
            queueSceneCue(session, SceneCueType.REACTION, "???", "?????????????????????", List.of("?????????", "????????????????"));
        }
        if (containsAny(normalizedMessage, "???", "??", "??")) {
            revealClueIfAvailable(session, "summer-clue-timeline-sketch");
            session.setCurrentStoryBeat("????????????????????????????????????????");
        }
        if (containsAny(normalizedMessage, "????", "????", "????")) {
            revealClueIfAvailable(session, "summer-clue-full-audio");
            queueSceneCue(session, SceneCueType.FORESHADOW, "????", "???????????????????????????????", List.of("????????????????", "?????????????????"));
        }
    }

    private void revealClueIfAvailable(GameSession session, String clueId) {
        StageDefinition currentStage = getCurrentStageDefinition(session);
        if (currentStage == null || !currentStage.getAvailableClueIds().contains(clueId)) {
            return;
        }
        ...
        state.setRevealed(true);
        state.setRevealedAt(LocalDateTime.now());
        if (!session.getPendingClueIds().contains(clueId)) {
            session.getPendingClueIds().add(clueId);
        }
    }

    private void queueSceneCue(GameSession session, SceneCueType type, String title, String content) {
        queueSceneCue(session, type, title, content, List.of());
    }

    private void queueSceneCue(GameSession session, SceneCueType type, String title, String content, List<String> quickActions) {
        ...
        pendingCues.add(new SceneCue(type, title, content, quickActions == null ? List.of() : quickActions));
    }
